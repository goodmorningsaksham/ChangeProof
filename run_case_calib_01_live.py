"""End-to-end live runner for case-calib-01 (Frozen Calibration Validation)."""
import os
import sys
import time
import json
...
OUTPUT_DIR = "runs/run_case_calib_01"
...
# PR diff: RETRIES_MAX=8, TIMEOUT=0.3, BACKOFF=0.0
...
+RETRY_TIMEOUT_SECONDS = float(os.getenv("RETRY_TIMEOUT_SECONDS", "0.3"))
...
spec = synth.synthesize(diff_text, case_id="case-calib-01", git_commit="calib-timeout-0.3")

with open(os.path.join(OUTPUT_DIR, "experiment.yaml"), "w", encoding="utf-8") as f:
    yaml.dump(spec, f, sort_keys=False)

# Step A: Apply calibration PR configuration (RETRIES_MAX=8, TIMEOUT=0.3, BACKOFF=0.0)
pr_code = (
    orig_code
    .replace('RETRIES_MAX = int(os.getenv("RETRIES_MAX", "3"))', 'RETRIES_MAX = int(os.getenv("RETRIES_MAX", "8"))')
    .replace('RETRY_TIMEOUT_SECONDS = float(os.getenv("RETRY_TIMEOUT_SECONDS", "1.0"))', 'RETRY_TIMEOUT_SECONDS = float(os.getenv("RETRY_TIMEOUT_SECONDS", "0.3"))')
...
print("Rebuilding checkout-service with calibration PR configuration (RETRIES_MAX=8, TIMEOUT=0.3, BACKOFF=0.0)...")
...
# Phase 3: BASE execution under 1500ms latency (from frozen formula max(2*300, 1500) = 1500ms)
print("\n=== EXECUTING BASE STATE (case-calib-01) ===")
toxi = ToxiproxyClient(admin_url="http://localhost:8474")
toxi.reset()
toxi.add_latency("payment-proxy", latency_ms=1500, jitter_ms=75)
...
print("\n=== EXECUTING PATCHED STATE (case-calib-01) ===")
remediated_code = (
    pr_code
    .replace('RETRIES_MAX = int(os.getenv("RETRIES_MAX", "8"))', 'RETRIES_MAX = int(os.getenv("RETRIES_MAX", "2"))')
    .replace('RETRY_TIMEOUT_SECONDS = float(os.getenv("RETRY_TIMEOUT_SECONDS", "0.3"))', 'RETRY_TIMEOUT_SECONDS = float(os.getenv("RETRY_TIMEOUT_SECONDS", "1.0"))')
...
# Phase 6: Multi-Signal Hypothesis Evaluation & Certificate
signals = [
...
candidate_hypos = generate_candidate_hypotheses(signals, proxy_name="payment-proxy", calibrated_latency_ms=1500)
...
cert_gen.generate_and_save({
    ...
    "pre_summary": base_summary,
    "post_summary": patched_summary,
    "capsule_path": "capsules/case-calib-01.zip",
}, cert_path)
...
capsule_zip = packager.create_capsule(
    experiment_id="case-calib-01",
    run_dir=OUTPUT_DIR,
    git_commit_base="calib-timeout-0.3",
)
