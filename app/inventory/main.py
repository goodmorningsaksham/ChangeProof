...
RETRY_EXHAUSTED_TOTAL = Counter(
    ...
    "Total times retry budget was exhausted in inventory",
    ["service", "target"],
)

# Initialize metric labels
RETRY_COUNT_TOTAL.labels(service="inventory", target="warehouse").inc(0)
RETRY_EXHAUSTED_TOTAL.labels(service="inventory", target="warehouse").inc(0)
...
def call_warehouse_with_retries(item_id: str, quantity: int, order_id: str, force_failure: bool = False) -> tuple[dict, int]:
    ...
    try:
        ...
        RETRY_EXHAUSTED_TOTAL.labels(service="inventory", target="warehouse").inc()
        raise e
...
def check_and_reserve(req: CheckReserveRequest):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Warehouse service timeout or unreachable: {str(e)}",
        )
    except Exception as e:
        INVENTORY_REQUESTS_TOTAL.labels(status="internal_error").inc()
        duration = time.time() - start_time
        INVENTORY_LATENCY_SECONDS.observe(duration)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Inventory processing error: {str(e)}",
        )
