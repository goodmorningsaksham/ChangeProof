...
- **Real Fault Injection via Toxiproxy (Not Simulation)**: Injects real network latency, packet loss, and connection resets into TCP streams rather than mocking HTTP responses (*see `docs/SUBMISSION_STATUS.md`, `case-01`, `case-10`*).
...
The repository contains 18 capsule archives representing 16 distinct evaluation scenarios (15 executed reproduction capsules covering canonical storms, holdout parameters, topology variations, calibrated latency bounds, negative controls, and multi-attempt self-correction, plus `case-05` as a safe negative control).
