...
"""
    ...
    import pandas as pd
    # Pre-patch failure reproduction (amplification: 2000 retries / 400 requests = 5.0 retries/req)
    pd.DataFrame([
        ...
        {"timestamp": 1.0, "metric_name": "checkout_requests_total", "service": "checkout", "target": "all", "value": 0.0},
        {"timestamp": 45.0, "metric_name": "checkout_requests_total", "service": "checkout", "target": "all", "value": 400.0},
    ]).to_csv(pre_csv, index=False)

    # Post-patch remediated state (bounded: 400 retries / 400 requests = 1.0 retry/req)
    pd.DataFrame([
        {"timestamp": 1.0, "metric_name": "retry_count_total", "service": "checkout", "target": "payment", "value": 0.0},
        {"timestamp": 45.0, "metric_name": "retry_count_total", "service": "checkout", "target": "payment", "value": 400.0},
        {"timestamp": 1.0, "metric_name": "checkout_requests_total", "service": "checkout", "target": "all", "value": 0.0},
        {"timestamp": 45.0, "metric_name": "checkout_requests_total", "service": "checkout", "target": "all", "value": 400.0},
